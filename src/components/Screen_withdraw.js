...
export default function Screen_withdraw(token){
    ...
    useEffect(() => {
        if (callAPI){
            send_balance({value:value,title:title,type:'withdraw'},token)
            .then((res) => {
                setIsLoading(false);
                navigate('/carteira')
            })
    ...
    return (
        <div className='root2'>
            <div className="deposit_withdraw_title">Nova Saída</div> 
